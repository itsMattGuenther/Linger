...
- **Signing out of one server isn't connected.** Settings has a Servers
  ...
  on its own in Servers (SIGN-13, MULTI-4).
...
| MULTI-4 | The order of servers is yours, and never reshuffles by activity. | `session.ts` | Move up and down in the server menu and Settings → Servers | F | 🟡 the server menu (next-list-window.spec.ts); Settings → Servers isn't wired |
